...
function amplification_factor(lambda::N, X::T) where {T<:AbstractRKTemporal, N<:Number}
    e = ones(eltype(X.b), size(X.b))
    I = UniformScaling(1)
    1 + lambda*transpose(X.b)*inv(I - lambda*X.A)*e
end

function amplification_factor(lambda::Matrix{N}, X::T) where {T<:AbstractRKTemporal, N<:Number}
...
function pseudo_source_factor(lambda::N, omega, dtau, X::T) where {T<:AbstractRKTemporal, N<:Number}
    e = ones(eltype(X.b), size(X.b))
    I = UniformScaling(1)
